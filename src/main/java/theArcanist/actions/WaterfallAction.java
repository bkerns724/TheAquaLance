...
public class WaterfallAction extends AbstractGameAction {
    ...
    public void update() {
        if (shouldCancelAction())
            ...
            tickDuration();
            if (isDone) {
                AbstractDungeon.effectList.add(new FlashAtkImgEffect(target.hb.cX, target.hb.cY, ArcanistMod.Enums.WATER));
                target.damage(info);
                ...
                if (AbstractDungeon.getCurrRoom().monsters.areMonstersBasicallyDead())
                    ...
                    vfxTop(new FlashAtkImgEffect(target.hb.cX, target.hb.cY, ArcanistMod.Enums.ICE));
