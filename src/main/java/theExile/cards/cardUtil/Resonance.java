...
public class Resonance {
    ...
    public void resonanceEffects(AbstractResonantCard card, AbstractMonster m) {
        ...
        if (adp() != null && adp().hasPower(AcousticsPower.POWER_ID)) {
            ...
            if (strength > 0)
                applyToSelf(new StrengthPower(adp(), strength));
            if (draw > 0)
                Wiz.cDraw(draw);
            if (discard > 0)
                Wiz.discard(discard);
    ...
    public String getDescription() {
        ...
        if (strength > 0) {
            if (started)
                builder.append(" NL ");
            started = true;
                builder.append(uiStrings.TEXT[9].replace("!X4!", String.valueOf(strength)));
        }
        if (draw > 0) {
            if (started)
                builder.append(" NL ");
    ...
    private String getConciseDescription() {
        ...
        if (strength > 0) {
            builderSpacer(builder, spaceCount);
            spaceCount++;
            builder.append(uiStringsConcise.TEXT[6].replace("!X4!", String.valueOf(strength)));
        }
        if (draw > 0) {
            builderSpacer(builder, spaceCount);
            spaceCount++;
